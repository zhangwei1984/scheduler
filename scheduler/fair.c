...
void gang_sched(struct task_group *tg, struct rq *rq)
{
	...
	if (rq->gang_leader == 1) {
        ...
        /* Use fixed cpumask instead of CPU Schedule Domain */

		if (cpu_of(rq) == cpumask_first(to_cpumask(&(tg->cpumask)))) {
			printk(KERN_INFO "Selected CPU %d as gang leader\n",
					cpu_of(rq));
			rq->gang_leader = 1;
			rq->gang_cpumask = to_cpumask(&(tg->cpumask));
		} else {
			/*
...
int sched_group_set_cpumask(struct task_group *tg, unsigned long cpumask)
{
    /*
        * root cgroup cannot be gang scheduled
    */
